...
%create an initial model to find an initial error
model= svmTrain(X, y, C, @(x1, x2) gaussianKernel(x1, x2, sigma)); 
predictions=svmPredict(model,Xval);
error=mean(double(predictions ~= yval));


%looping over all of the Cs and sigmas to find the minimum error
for C_i=[0.01 0.03 0.1 0.3 1 3 10 30]
  for sigma_i=[0.01 0.03 0.1 0.3 1 3 10 30]
    ...
    %if the error is smaller replace C and sigma
    if (error_i<error)
      C=C_i;
      sigma=sigma_i;
      error=error_i;
